...
function Main() {
    ...
    const handleScroll = () => {
      ...
      const visibleComponent = positions.find((position) => {
        if (position && position?.name == "Introduction") {
          return (
            position &&
            position.top <= scrollY + windowHeight / 2 &&
            position.bottom >= scrollY + windowHeight / 2 &&
            position.right <= windowWidth &&
            position.right >= windowWidth / 3
          );
  ...
  return (
    <div>
      ...
      {/* <div
        className={`${
          currentComponent !== "Home" && currentComponent !== null
            ? "fixed bottom-0 z-40 backdrop-blur-xl bg-white/30 h-[44px] w-[100%] md:hidden"
            : "hidden"
        }`}
      ></div> */}
      {/* <Vision /> */}
